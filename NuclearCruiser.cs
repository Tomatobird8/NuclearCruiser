...
public class NuclearCruiser : BaseUnityPlugin
{
    ...
    public void Awake()
    {
        ...
        else
        {
            ...
            nukeLightIntensity = Config.Bind("General", "NukeLightIntensity", 1f, new ConfigDescription("Intensity of the light from the nuclear explosion.", new AcceptableValueRange<float>(0f, 1f)));
            ...
            shakeCamera = Config.Bind("General", "ShakeCamera", true, "Should camera shake upon explosion?");
            decalVisibilityRange = Config.Bind("General", "DecalVisibilityRange", 48f, "How far nuke explosion decals are visible.").Value;
            decalScale = Config.Bind("General", "DecalScale", 4f, "How large are the nuke explosion decals.").Value;
            protectionTime = Config.Bind("General", "ProtectionTime", 3f, "Amount of time in seconds the cruiser stays more protected from damage when detached from dropship.").Value;
            infiniteBoosts = Config.Bind("General", "InfiniteBoosts", true, "Should nuclear cruiser have infinite boosts?").Value;
            nuclearCruiserWarning = Config.Bind("General", "NuclearCruiserWarning", true, "Should a warning pop up when a Nuclear Cruiser is spawned?").Value;
            nuclearCruiserRadiationWarning = Config.Bind("General", "NuclearCruiserRadiationWarning", true, "Should a radiation warning pop up when a Nuclear Cruiser is spawned?").Value;
            ...
            minimumCrashVelocity = Config.Bind("General", "MinimumCrashVelocity", 4f, "Damaging impact velocity threshold. Default threshold is reached at very low speeds.").Value;
            crashDamage = Config.Bind("General", "CrashDamage", 4, "Amount of damage cruiser takes on impact. Only used when CruiserFragility is set to Fragile.").Value;
